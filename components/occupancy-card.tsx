...
import { TrendingDownIcon, ArrowRight, Loader2 } from "lucide-react";
...
export function OccupancyCard() {
  const [loadingViewVacant, setLoadingViewVacant] = useState(false);

  const handleViewVacant = () => {
    setLoadingViewVacant(true);
    setTimeout(() => setLoadingViewVacant(false), 2000);
  };
  return (
    <Card className="flex flex-col justify-between">
      ...
      <CardContent className="">
        <div className="grid grid-cols-2 divide-x rounded-md border">
          ...
          <div className="flex flex-col items-center justify-center py-6 gap-1">
            ...
            <span className="text-xs text-muted-foreground ">Vacant units</span>
          </div>
        </div>
      </CardContent>
      <CardFooter className="flex items-center justify-between">
        ...
        <Button
          variant="outline"
          className="gap-1.5 text-xs font-semibold h-8 px-3 cursor-pointer flex-1 sm:flex-initial min-w-36"
          disabled={loadingViewVacant}
